...
function wxSearchConfirm(e) {
  ...
  var inputValue = e.target.dataset.content;
  wx.request({
    url: 'https://rt.zhaiqihua.top/testWxprogram/search.php',
    data: { oname: inputValue},
    ...
    if(res.data.length!=0){
      wx.navigateTo({
        url: '../../pages/index/detail-buy/detail-buy?id=' + res.data[0].id,
      })
    }else{
      wx.showToast({
        title: '搜索失败',
